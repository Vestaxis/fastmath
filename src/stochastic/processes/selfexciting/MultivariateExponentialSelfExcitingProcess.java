...
//		}
	...
	private Vector calculateInitialGuess(Vector times, Vector types)
	{
		...
		kappa = params.slice(0, getDim());
		alpha = new DoubleMatrix[getOrder()];
		beta = new DoubleMatrix[getOrder()];
		for (int j = 0; j < getDim(); j++)
		{
			Vector theseTimes = new Vector(timesSub[j]);
			kappa.set(j, 0.5 / theseTimes.diff().mean());
		}

		for (int j = 0; j < getOrder(); j++)
		{
			alpha[j] = params.slice(getDim() + (j * matrixSize), getDim() + ((j + 1) * matrixSize)).reshape(getDim(),
					getDim());
			beta[j] = params.slice(getDim() + (getOrder() * matrixSize) + (j * matrixSize),
					getDim() + (getOrder() * matrixSize) + ((j + 1) * matrixSize)).reshape(getDim(), getDim());
			alpha[j].diag().assign(1.0 / getOrder());
			beta[j].assign(2.0);
		}
	...
	public Vector calculateMean()
	{
		try
		{
			return eye(getDim()).subtract(calculateBranchingMatrix()).ldivide(kappa.asMatrix().trans().copy(false))
					.asVector();
		}
	...
	public DoubleColMatrix calculateBranchingMatrix()
	{
		...
		for (int j = 0; j < getOrder(); j++)
		{
			DoubleMatrix alphaj = alpha[j];
			DoubleMatrix betaj = beta[j];
	...
	public Vector calculateIntensitySlow(Pair<Vector[], TreeMap<Double, Integer>[]> timesSubPair, int m)
	{
		...
		for (int i = 1; i < Nm; i++)
		{
			...
			for (int n = 0; n < getDim(); n++)
			{
				...
				for (int j = 0; j < getOrder(); j++)
				{
					final double alphajmn = alpha[j].get(m, n);
					final double betajmn = beta[j].get(m, n);
					double ntime;
	...
	public Vector calculateIntensity(Pair<Vector[], TreeMap<Double, Integer>[]> timesSubPair, int m)
	{
		...
		for (int i = 1; i < Nm; i++)
		{
			...
			double logsum = kappa.get(m) * getDeterministicIntensity(m, upperTime, i);
	 ...
	 * @return
	 */
	private Entry<Double, Integer> getUpperEntry(TreeMap<Double, Integer>[] subTimeIndex, final double upperTime, int n,
			int m, int i)
	{
	 ...
	 * @return
	 */
	private Entry<Double, Integer> getLowerEntry(TreeMap<Double, Integer>[] subTimeIndex, final double lowerTime, int n,
			int m, int i)
	{
		...
		for (int i = 1; i < N; i++)
		{
			...
			for (int n = 0; n < getDim(); n++)
			{
				...
				for (int j = 0; j < getOrder(); j++)
				{
					...
					{
						sum += (alphajmn / betajmn)
								* (exp(-betajmn * (lowerTime - ktime)) - exp(-betajmn * (upperTime - ktime)));
					}
	...
	public double predict(Pair<Vector[], TreeMap<Double, Integer>[]> timesSubPair, final int m)
	{
		final Vector[] timesSub = timesSubPair.left;
		final TreeMap<Double, Integer>[] subTimeIndex = timesSubPair.right;
		final double kappa = this.kappa.get(m);
