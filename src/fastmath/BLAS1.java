...
public class BLAS1
{
  ...
  /**
   * compute y := alpha * x + y where alpha is a scalar and x and y are
   * n-vectors.
   */
   ...
   * lambda(j) is its eigenvalue. The left eigenvector u(j) of A satisfies
   * u(j)**H * A = lambda(j) * u(j)**H where u(j)**H denotes the conjugate
   * transpose of u(j).
   * 
   ...
   *         converged.
   */
  public static int dgeev( AbstractMatrix A, Vector wr, Vector wi, AbstractMatrix vl, AbstractMatrix vr, Vector work, int workSize ) throws FastMathException
  {
   ...
   * Y = X
   * 
   * WARNING: X and Y cannot overlap unless they are both reversed ( not
   * verified yet )
   */
  public static void dcopy( Vector X, Vector Y )
  {
    ...
    dcopy( X.size(), X.getBuffer(), X.getOffset( 0 ), X.getIncrement(), Y.getBuffer(), Y.getOffset( 0 ), Y.getIncrement() );
  }
   ...
   *         elements of an intermediate bidiagonal form did not converge to
   *         zero.
   */
  public static int dgelss( DoubleMatrix A, DoubleMatrix B, Vector S, IntVector rank, Vector work, int workSize )
  {
    ...
    return dgelss( A.getRowCount(),
                   ...
                   S.getBuffer(),
                   S.getOffset( 0 ),
                   -1.0, // TODO: compute rcond, this defaults to machine
                   // precision
                   rank.getBuffer(),
                   work.getBuffer(),
                   workSize );
