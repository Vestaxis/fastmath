...
public class MatFile implements Iterable<MiElement>
{
   ...
   * indicates that the program reading the MAT-file must perform byte-swapping
   ...
   * following information in their headers: MATLAB version Platform on which
   * the file was created Date and time the file was created
   */
  ...
  /**
   * Open a file for WRITING, writes the 128 byte header, if exists, it is
   * renamed to the same filename with ".bak" appended and if the JVM exits
   * before this{@link #close()} is called, the original file will be restored
 ...
 * @throws IOException
  ...
  public MiIterator iterator()
  {
    try
    {
      MappedByteBuffer buffer = getFileChannel().map( MapMode.READ_ONLY, HEADER_OFFSET, getFileChannel().size() - HEADER_OFFSET );
  ...
  /**
   * Like this{@link #readVariables(String)} but filters variable names against
  ...
  /**
   * 
   * @return the number of times this
   *         {@link #write(String, MiWritableElement...)} was called
   * 
   */
