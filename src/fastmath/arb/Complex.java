...
public class Complex extends Structure
{
  ...
  public Complex approximationZero( int n )
  {
    ...
    final Complex ndx = new Complex( rat.divide( ( ComplexONE.exp().multiply( 8 ) ) ) );
    final Complex w = ndx.W();
    final Complex top = new Complex( PI * ( 8 * n - 11 ), 0 );
    return top.divide( ( w.multiply( 4 ) ) );
  }
  ...
  public Complex approximationZero( int n, Complex s )
  {
    ...
    final Complex ndx = new Complex( rat.divide( ( ComplexONE.exp().multiply( s8 ) ) ) );
    final Complex w = ndx.W();
  ...
  public Complex iterate( Iteration map, int n, double h, boolean print, Complex y, double target, double start, int index, List<Real> removedPoints )
  {
    ...
    for ( int i = 0; i < n; i++ )
    {
      ...
      double diff = z.getReal().subtract( pz.getReal() ).sixtyFourBitValue();
      if ( Math.abs( diff ) > 2 )
      {
      ...
      if ( i > 500 )
      {
        err.println( "slow convergence warning " + i + " pz=" + pz.getReal().sixtyFourBitValue() + " z=" + z.getReal().sixtyFourBitValue() + " overshot "
                     + overshot + " h=" + h + " index=" + index + " start=" + start + " target=" + target  );
      }
      ...
      if ( print )
      {
        println( z.getReal().sixtyFourBitValue() + " diff=" + diff + " pz=" + pz.getReal().sixtyFourBitValue() + " z=" + z.getReal().sixtyFourBitValue()
                 + " diff=" + diff + " start=" + start + " target=" + target + " index=" + index );
      }
   ...
   * (and neither contains NaN). To test whether both operands might represent
   * the same mathematical quantity, use acb_overlaps() or acb_contains(),
  ...
  public Real nthRiemannZeroArg( int n )
  {
    ...
    Real wtf = this.getReal()
                   .divide( ( RealConstants.PI.multiply( new Real( 2 ) ).multiply( RealConstants.HALF.exp() ) ).divide( ( RealConstants.PI.multiply( 2 ) ) )
                                                                                                               .add( new Real( 7 ).divide( new Real( 8 ) ) )
                                                                                                               .log() );
  ...
  {
    return HardyZDerivative().multiply( ComplexONE.subtract( HardyZTanh().pow( 2 ) ) );
  }

  public Complex HardyZTanhReducedDerivative( Complex s )
  {
    Complex tminuss = this.subtract( s );
    return HardyZTanhDerivative().divide( tminuss ).subtract( HardyZTanh().divide( tminuss.pow( 2 ) ) );
  }
