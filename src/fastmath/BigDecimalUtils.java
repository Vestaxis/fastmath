...
public class BigDecimalUtils
{
  ...
  private static BigDecimal lnNewton( BigDecimal x, int scale )
  {
    ...
    while (term.compareTo( tolerance ) > 0 && !Thread.interrupted());
  ...
  {
    final int NUM_OF_DIGITS = dp + 2; // need to add one to get the right number
                                      // of dp
                                      // and then add one again to get the next
                                      // number
