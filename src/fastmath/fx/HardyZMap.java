...
public class HardyZMap extends Application
{
  private double yrange = 3;
  private double xrange = 26;
  private double lineWidth = sqrt( pow( yrange, 2 ) + pow( yrange, 2 ) ) / 2000;
 
  private double minx = 10;
  // private static final double minx = 280;
  private double miny = -yrange / 2;
  private double maxy = yrange / 2;

  private Iteration iteratedMap = Iteration.ZHTEven;
  ...
  private double maxx = minx + xrange;
  ...
  {
    out.println( "Starting " + primaryStage );
    stage = primaryStage;
    xrange = maxx - minx;
    yrange = maxy - miny;
  ...
  private boolean fitAspectRatio = true;
  ...
  protected void tracePath( MouseEvent event )
  {
     ...
     */
    int idx = HardyZMap.mapPointToNearestIndex( re );
    double nr = HardyZMap.Zroots[idx];
  ...
  protected void render()
  {
    ...
    new Thread( () -> {
      ...
      mapSurfaceRegionToColoredPixelGrid( gc );
      drawPointsAndLines( Color.BLACK, gc );
  ...
  protected void setupCoordinateSystem()
  {
    ...
    if ( fitAspectRatio )
    {
      ...
      miny = -yrange / 2;
      maxy = yrange / 2;
      out.println( "yrange set to " + yrange + " to conserve aspect ratio " + aspectRatio );
    }
  ...
  protected void mapSurfaceRegionToColoredPixelGrid( GraphicsContext gc )
  {
    // if ( painter == null )
    // {
    ...
    convergents = new int[pwidth][pheight];

    ExponentialMovingAverage ewma = new ExponentialMovingAverage( 0.1 );
    final int[] cnt = new int[]
    {
      (int) width
    };
    rendering = true;
    range( 0, pwidth ).boxed().sorted( shuffler ).parallel().forEach( px -> {
      if ( !cancelRendering )
      {
        long startTime = currentTimeMillis();
        drawVerticalLine( shuffler, pheight, px );
        long stopTime = currentTimeMillis();
        int threadCount = Runtime.getRuntime().availableProcessors();
        double avg = ewma.average( stopTime - startTime ) / ( threadCount * 1000.0 );
        double remaining = ( ( ( cnt[0]-- ) * avg ) ) / ( 60.0 );
        out.println( "ETA=" + remaining + " minutes at a rate of 1 vertical stripe every " + avg + " seconds" );
      }
    } );
    rendering = false;
    if ( cancelRendering )
    {
      println( "Rendering cancelled (because of zoom)" );
    }
  }
  ...
  protected void drawVerticalLine( RandomComparator<Integer> shuffler, int pheight, int px )
  {
    double z[] = new double[2];

    range( 0, pheight ).boxed().sorted( shuffler ).parallel().forEach( py -> {
      if ( !cancelRendering )
      {
        double initx = mapPixelToRealPart( px );
        double inity = mapPixelToImagPart( py );
        ...
        Color color = getIndexColor( index );
        if ( converges )
        {
          double brightnessFactor = max( 0.25, 1 - ( (double) log( 1 + iterationCount / 10.0 ) ) );
          // double brightnessFactor = exp( -1 / ( 1 + iterationCount ) );
          color = color.deriveColor( 0, 1, brightnessFactor, 1 - scaledb );
        }
        convergents[px][py] = index;
        final Color pixelColor = color;
        Platform.runLater( () -> {
          gc.getPixelWriter().setColor( px, py, pixelColor );
        } );
        // points.add( new Point( px, py, converges ? color : Color.WHITE ) );
      }
    } );
  }

  public Color[] colorCycle;
  private static double ballRadius = 0.25;
  ...
  protected void drawPointsAndLines( Color color, GraphicsContext gc )
  {
    ...
    boolean mirrored = abs( miny ) == abs( maxy );
    if ( mirrored )
    {
      sketchZFunction( color, gc, z );
    ...
    ballRadius = min( abs( xrange ) / 100, abs( yrange ) / 100 );
    // lineWidth = ballRadius / 100;
    for ( double zroot : HardyZMap.Zroots )
    {
  ...
  protected void sketchRealPartOfZetaFunction( Color color, GraphicsContext gc, double[] z )
  {
    Functions.ζ( 0.5, minx, z );
    gc.setStroke( color );
    gc.setLineWidth( lineWidth );
    gc.beginPath();
    gc.moveTo( 0.5, minx );
    for ( double t = minx; t < maxx; t += 0.01 )
    {
      Functions.ζ( 0.5, t, z );
      gc.lineTo( t, z[0] );
    }
  ...
  protected void sketchImaginaryPartOfZetaFunction( Color color, GraphicsContext gc, double[] z )
  {
    Functions.ζ( 0.5, minx, z );
    gc.setStroke( color );
    gc.setLineWidth( lineWidth );
  ...
  protected void sketchZFunction( Color color, GraphicsContext gc, double[] z )
  {
    Functions.Z( minx, 0, z );
    gc.setStroke( color );
    gc.setLineWidth( lineWidth );
    gc.beginPath();
    gc.moveTo( minx, 0 );
  ...
  static
  {
    for ( int i = 0; i < HardyZMap.Zroots.length; i++ )
    {
      Zidx.put( HardyZMap.Zroots[i], i );
    }
  }
