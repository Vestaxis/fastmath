...
public class ColorUtils
{
   ...
   * the rest as possible and each color has at least one component greater than
   * minComponent and one less than maxComponent. Use min == 1 and max == 0 to
  ...
  public static Color[] generateVisuallyDistinctColors( int ncolors, float minComponent, float maxComponent )
  {
    ...
    for ( int i = 0; i < yuv.length; i++ )
    {
      ...
      rgbs[i] = Color.color( rgb[0], rgb[1], rgb[2] );
  ...
  private static float[] randYUVinRGBRange( float minComponent, float maxComponent )
  {
    while (true)
    {
      ...
      if ( 0 <= r && r <= 1 && 0 <= g && g <= 1 && 0 <= b && b <= 1 && ( r > minComponent || g > minComponent || b > minComponent ) && // don't
                                                                                                                                       // want
                                                                                                                                       // all
                                                                                                                                       // dark
                                                                                                                                       // components
  ...
  private static double worstFit( Color[] colors )
  {
    ...
    {
      a[0] = (float) colors[i].getRed();
      a[1] = (float) colors[i].getGreen();
      a[2] = (float) colors[i].getBlue();
      for ( int j = 0; j < i; j++ )
      {
        b[0] = (float) colors[j].getRed();
        b[1] = (float) colors[j].getGreen();
        b[2] = (float) colors[j].getBlue();
  ...
  private static float[] randYUVBetterThan( float bestDistSqrd, float minComponent, float maxComponent, float[][] in )
  {
    ...
    return null; // after a bunch of passes, couldn't find a candidate that beat
                 // the best.
  }
