...
public class AutoHashMap<K, V> extends ConcurrentHashMap<K, V> implements AutoMap<K, V>
{
  ...
  public V getOrCreate(K key)
  {
    ...
    if (value == null)
    {
      synchronized (this)
      {
        try
        {
          V previousValue = putIfAbsent(key, value = (valueClass == null ? newValueInstance(key) : (V) valueClass.getConstructor().newInstance()));
  ...
  {
    if (constructor != null) { return constructor.apply(key); }
